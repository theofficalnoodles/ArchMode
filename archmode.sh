...
reset_all() {
    ...
    echo -e "${GREEN}✓ System reset to defaults${NC}"
    log "System reset to defaults"
}
...
# =========================
show_help() {
    echo -e "${BOLD}ArchMode v${VERSION}${NC}"
    echo ""
    echo "Usage:"
    echo "  archmode <command> [option]"
    echo ""
    echo "Commands:"
    ...
    echo "  enable <MODE>           Toggle a mode (e.g. GAMEMODE)"
    ...
    echo "  plugin start <NAME>     Start plugin"
    echo "  plugin stop <NAME>      Stop plugin"
    echo "  plugin status <NAME>    Plugin status"
...
case "${1:-}" in
    ...
    plugin)
        case "${2:-}" in
            start) plugin_start "${3:-}" ;;
            stop) plugin_stop "${3:-}" ;;
            status) plugin_status "${3:-}" ;;
            *)
                echo -e "${RED}✗ Invalid plugin command${NC}"
                ;;
        esac
        ;;
    help|"")
